...
def geographic2pixel(img, lon, lat):
    """
    Convert given (x,y) in pixels to +VE longitude and latitude. 
    ...
    """
    assert lon < 360 and lon >= 0, 'Error; longitude value out of bounds.'
    assert lat < 90 and lat > -90, 'Error; latitude value out of bounds.'
    ...
    lon_step, lat_step = hdulist[0].header['LON_STEP'], hdulist[0].header['LAT_STEP']
    lon_left, lat_bot = hdulist[0].header['LON_LEFT'], hdulist[0].header['LAT_BOT']
    x, y = int((lon-lon_left)/lon_step), int((lat- lat_bot)/lat_step)
...
def overlap_all(lat, v, path2data='./'):
    ...
    y = geographic2pixel(images[0],0,lat)[1]
...
def averaging_correlation_img_pair(lat, v, img1, img2, N=5):
    ...
    # Convert latitude to pixel, note longitude does not matter 
    y = geographic2pixel(img1,0,lat)[1]
...
def readZWP(path2wp, plotting=False): 
    '''
...
def v_maxcorr(lat, path2data=None, plotting=False, vstep=37):
    ...
    im_pairs = overlap_all(lat, 0, path2data=path2data)
